...
class StudentProfile(models.Model): # Model para StudentProfile - informações pessoais do estudante, um perfil estendido.
    ...
    def __str__(self): # __str__ é usado para retornar uma string que representa o objeto
        return f"{self.user.get_full_name() or self.user.username} - {self.area_interesse or 'Sem área definida'}" # Exemplo: "Mariana Menezes - Arquitetura"
    ...
    def calcular_completude_perfil(self): # Percentual de completude do perfil
        ...
        preenchidos = sum(1 for campo in campos_obrigatorios if campo) # 1 for campo in campos_obrigatorios if campo: para cada campo em campos_obrigatorios, se o campo for preenchido, soma 1
        return int((preenchidos / len(campos_obrigatorios)) * 100) # A partir daqui é possível calcular o percentual de completude do perfil
    
    def save(self, *args, **kwargs): # Override save para atualizar perfil_completo automaticamente. '*' são ponteiros? Sim, são ponteiros para os argumentos passados para o método.
        ...
        super().save(*args, **kwargs) # super() é usado para chamar o método save do modelo pai. Como assim? Porque o método save do modelo pai é o método que salva o objeto no banco de dados.
...
class Resume(models.Model): # Model para Resume - informações do currículo do estudante
    ...
    def __str__(self):
        return f"Currículo de {self.student.user.get_full_name() or self.student.user.username}" # Exemplo: "Currículo de Mariana Menezes"
...
class WorkExperience(models.Model): # Model para WorkExperience - informações da experiência profissional do estudante
    ...
    resume = models.ForeignKey( # ForeignKey é usado para criar um relacionamento com o modelo Resume. Importante para a realização do modelo conceitual.
        ...
        related_name='experiencias', # related_name: nome do relacionamento com o currículo. "experiencias" = experiências profissionais do estudante
...
class Education(models.Model): # Model para Education - informações da formação acadêmica do estudante
    ...
    nivel = models.CharField(
        max_length=20,
        choices=[
            ('medio', 'Ensino Médio'), # Por que ([x], [X])? Porque o campo é um choices, ou seja, só pode ser um dos valores listados.
            ('tecnico', 'Técnico'),
            ('graduacao', 'Graduação'),
            ('pos', 'Pós-Graduação'),
            ('mestrado', 'Mestrado'),
...
class StudentSkill(models.Model): # Model para StudentSkill - informações das habilidades do estudante. Relacionamento ManyToMany customizado entre Estudante e Habilidadescom nível de proficiência
    ...
    def __str__(self):
        return f"{self.student.user.username} - {self.skill.nome} ({self.get_nivel_display()})" # Exemplo: "Mariana Menezes - Design de Interiores (Intermediário)"
...
class JobOpportunity(models.Model): # Model para JobOpportunity - informações das vagas de emprego disponíveis para estudante
    ...
    logo_empresa = models.ImageField( # ImageField é usado para armazenar imagens.
        upload_to='companies/logos/', # upload_to é usado para especificar o diretório onde as imagens serão armazenadas. Onde ficaram armazenadas? No diretório 'companies/logos/' dentro da pasta 'backend/media/'.
        null=True,
        blank=True,
